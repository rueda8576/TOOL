...
describe("processEmailJob", () => {
    ...
    await processEmailJob(
      prisma,
      {
        data: {
          ...
          directEmail: {
            to: "user@example.com",
            subject: "Atlasium service notice",
            text: "A service notice is ready for review."
          }
        }
      } as any
    );
  ...
  it("decrypts encrypted direct emails before delivery and marks the event as sent", async () => {
    const sendMail = jest.fn().mockResolvedValue(undefined);
    ...
    const encryptedDirectEmail = encryptValue(
      JSON.stringify({
        to: "user@example.com",
    ...
    await processEmailJob(
      prisma,
      {
        data: {
          ...
          encryptedDirectEmail
        }
      } as any
    );

    expect(JSON.stringify({ encryptedDirectEmail })).not.toContain("/reset-password?token=");
    expect(sendMail).toHaveBeenCalledWith({
      from: "no-reply@atlasium.info",
      to: "user@example.com",
      subject: "Atlasium password reset",
  ...
  it("marks encrypted direct emails as failed when the payload cannot be decrypted", async () => {
    const sendMail = jest.fn().mockResolvedValue(undefined);
    const { processEmailJob } = await loadJob(sendMail);
    const prisma = {
      notificationEvent: {
    ...
    await expect(
      processEmailJob(
        prisma,
        {
          data: {
            notificationEventId: "event-reset",
            encryptedDirectEmail: "bad-payload"
          }
        } as any
      )
    ...
    expect(prisma.notificationEvent.update).toHaveBeenCalledWith({
      ...
      data: {
        status: NotificationStatus.FAILED,
        errorMessage: "Unsupported encrypted payload format"
      }
    });
  });
