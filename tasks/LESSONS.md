...
- For Windows HTTPS clone against self-managed GitLab, Git Credential Manager can use the GitLab provider with browser auth (`credential.git.atlasium.info.provider=gitlab`, `credential.gitLabAuthModes=browser`); stale host credentials must be erased before retesting.
